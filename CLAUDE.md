...
- **Test/** — Test framework and themed test modules (97 tests). `index.ts` is a thin barrel that imports the modules in order; tests live in `name.ts`, `packet.ts`, `edns.ts`, `recordTypes.ts`, `proxyProtocol.ts`, `tsig.ts`, `zoneParser.ts`, `serverDoh.ts`, `serverIntegration.ts`, `serverTls.ts`. Shared fixtures (`response` buffer, HTTP `get` helper, bundled TLS cert/key, `dotQuery` helper) live in `helpers.ts`. The `zoneParser.ts` module ships a `dedent` helper for indented multi-line zone literals — RFC 1035 leading-whitespace semantics conflict with template-literal aesthetics, so test inputs are dedented before parsing.
