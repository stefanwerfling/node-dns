...
- **Client/** — `UDPClient`, `TCPClient` (TCP + TLS), `DohClient` (http/https/h2), `GoogleClient` (Google JSON API), `AxfrClient` (RFC 5936 zone transfer over TCP/TLS — reassembles multi-message replies, terminates on the closing SOA). All expose a static `request()` method returning a resolver function.
- **DNS.ts** — High-level resolver that tries multiple nameservers in parallel, supports all protocol types.
- **Test/** — Test framework and themed test modules (104 tests). `index.ts` is a thin barrel that imports the modules in order; tests live in `name.ts`, `packet.ts`, `edns.ts`, `recordTypes.ts`, `proxyProtocol.ts`, `tsig.ts`, `zoneParser.ts`, `axfr.ts`, `serverDoh.ts`, `serverIntegration.ts`, `serverTls.ts`. Shared fixtures (`response` buffer, HTTP `get` helper, bundled TLS cert/key, `dotQuery` helper) live in `helpers.ts`. The `zoneParser.ts` module ships a `dedent` helper for indented multi-line zone literals — RFC 1035 leading-whitespace semantics conflict with template-literal aesthetics, so test inputs are dedented before parsing.
