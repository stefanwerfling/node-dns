...
const buildPositive = (name, addr, ttl = 60) => {
    const p = new Packet();
    p.header.qr = 1;
    p.header.aa = 1;
...
const buildNxdomain = (name, soaTtl = 30) => {
    const p = new Packet();
    p.header.qr = 1;
    p.header.aa = 1;
...
const buildNodata = (name, soaTtl = 30) => {
    const p = buildNxdomain(name, soaTtl);
    p.header.rcode = RCODE.NOERROR;
    return p;
};
const buildServfail = (name) => {
    const p = new Packet();
    p.header.qr = 1;
...
const countingBackend = (router) => {
    let calls = 0;
    const backend = async (name) => {
        calls++;
        return router(name);
    };
    return { backend: backend, callCount: () => calls };
};
test('CachedStubBackend caches NOERROR answers and short-circuits next call', async () => {
    ...
    assert.equal(r2.answers[0].packetType.address, '192.0.2.1');
...
test('CachedStubBackend decrements TTL on serve', async () => {
    ...
    const cached = new CachedStubBackend(backend, { cache: cache, now: () => now });
...
test('CachedStubBackend serves stale + schedules refresh within maxStaleSeconds window', async () => {
    let now = 1_700_000_000_000;
    const cache = new DnsCache({ now: () => now, maxStaleSeconds: 60 });
    let calls = 0;
    ...
    const cached = new CachedStubBackend(upstream, { cache: cache, now: () => now });
    await cached.resolve('www.example.com', PacketTypes.A, PacketClass.IN);
    assert.equal(calls, 1);
    now += 90_000;
    nextAddr = '192.0.2.2';
    const r = await cached.resolve('www.example.com', PacketTypes.A, PacketClass.IN);
    assert.equal(r.answers[0].packetType.address, '192.0.2.1');
    await new Promise((resolve) => setImmediate(resolve));
    assert.equal(calls, 2, 'background refresh must have fired');
    const r2 = await cached.resolve('www.example.com', PacketTypes.A, PacketClass.IN);
    assert.equal(r2.answers[0].packetType.address, '192.0.2.2');
...
test('CachedStubBackend throws on missing upstream', () => {
    assert.throws(() => new CachedStubBackend(undefined), /upstream backend is required/);
});
...
test('SystemResolver wires cache:true behind HostsFile (hosts still authoritative)', async () => {
    ...
    assert.equal(r1.answers[0].packetType.address, '10.0.0.5');
...
test('SystemResolver accepts cache: DnsCacheOptions and creates a configured cache', async () => {
    const upstream = async (name) => buildPositive(name, '203.0.113.1', 300);
    const resolver = SystemResolver.fromConfig({
        nameservers: ['1.1.1.1'],
        search: [],
        sortlist: [],
