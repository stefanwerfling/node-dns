...
test('zone#single A record with origin', () => {
    ...
    assert.equal(records[0].packetType.address, '192.0.2.1');
});
...
test('zone#multi-line record via parens (SOA)', () => {
    ...
    const { records } = ZoneParser.parse(dedent(zone));
    assert.equal(records.length, 1);
    const soa = records[0].packetType;
...
test('zone#TXT quoted string preserves spaces and ; ', () => {
    ...
    `;
    const { records } = ZoneParser.parse(dedent(zone));
    assert.equal(records[0].packetType.data, 'hello; world with spaces');
});
test('zone#TXT multiple character-strings', () => {
    ...
    `;
    const { records } = ZoneParser.parse(dedent(zone));
    const txt = records[0].packetType;
    assert.deepEqual(txt.data, ['v=spf1', 'ip4:192.0.2.0/24', '-all']);
});
test('zone#TXT with escaped quote inside string', () => {
    const { records } = ZoneParser.parse('host 60 IN TXT "she said \\"hi\\""', { origin: 'example.com.' });
    assert.equal(records[0].packetType.data, 'she said "hi"');
});
test('zone#each common record type', () => {
    ...
    assert.equal(records[1].packetType.ns, 'ns1.example.com');
    const mx = records[2].packetType;
    assert.equal(mx.priority, 10);
    assert.equal(mx.exchange, 'mail.example.com');
    assert.equal(records[3].packetType.address, '192.0.2.1');
    assert.equal(records[4].packetType.address, '2001:db8::1');
    assert.equal(records[5].packetType.domain, 'www.example.com');
    assert.equal(records[6].packetType.domain, 'www.example.com');
    const srv = records[7].packetType;
    assert.equal(srv.priority, 10);
    assert.equal(srv.weight, 20);
    assert.equal(srv.port, 80);
    assert.equal(srv.target, 'www.example.com');
    const caa = records[8].packetType;
...
test('zone#CDS parses with same fields as DS', () => {
    const { records } = ZoneParser.parse('@ 3600 IN CDS 31589 8 2 AABBCCDDEEFF00112233445566778899AABBCCDDEEFF00112233445566778899', { origin: 'example.com.' });
    assert.ok(records[0].packetType instanceof CDS);
    const cds = records[0].packetType;
    assert.equal(cds.keyTag, 31589);
    assert.equal(cds.algorithm, 8);
    assert.equal(cds.digestType, 2);
    assert.equal(cds.digest, 'aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899');
});
test('zone#CDS delete sentinel (0 0 0 00) parses', () => {
    const { records } = ZoneParser.parse('@ 3600 IN CDS 0 0 0 00', { origin: 'example.com.' });
    const cds = records[0].packetType;
...
test('zone#CDNSKEY parses with same fields as DNSKEY', () => {
    ...
    const cdnskey = records[0].packetType;
