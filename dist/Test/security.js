...
test('client/0x20#mismatch throws', async () => {
    const server = new DnsServer({
        ...
        handle: (request, send) => {
            ...
            response.questions = request.questions.map((q) => new PacketQuestion(q.name.toLowerCase(), q.type, q.class));
            response.answers.push(new PacketResource(request.questions[0].name, new A('192.0.2.1'), PacketClass.IN, 60));
            send(response);
        },
    });
    ...
    await assert.rejects(resolve('LongTestName.example.com', PacketTypes.A, PacketClass.IN), /0x20 mismatch/);
    await server.close();
});
test('client/0x20#preserves case when server echoes correctly', async () => {
    ...
    assert.equal(result.answers[0].packetType.address, '192.0.2.1');
    await server.close();
});
