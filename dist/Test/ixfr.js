...
test('ixfr#makeQuery puts current SOA in authority section (RFC 1995 §3)', () => {
    ...
    assert.equal(parsed.authorities[0].packetType.serial, 1);
});
test('ixfr#zone.toIxfrPackets returns single SOA when serial matches', () => {
    ...
    assert.equal(response.answers[0].packetType.serial, 1);
});
...
test('ixfr#zone.toIxfrPackets emits incremental diff when history covers gap', () => {
    ...
    assert.equal(response.answers[0].packetType.serial, 2);
    assert.equal(response.answers[1].packetType.serial, 1);
    assert.equal((response.answers[2].packetType).address, '192.0.2.1');
    assert.equal(response.answers[3].packetType.serial, 2);
    assert.equal((response.answers[4].packetType).address, '192.0.2.99');
    assert.equal((response.answers[5].packetType).address, '192.0.2.100');
    assert.equal(response.answers[6].packetType.serial, 2);
});
test('ixfr#client end-to-end no-change response', async () => {
    ...
    if (result.type === 'noChange') {
        assert.equal(result.soa.packetType.serial, 1);
    }
    await server.close();
});
test('ixfr#client end-to-end AXFR fallback', async () => {
    ...
    if (result.type === 'fullAxfr') {
        assert.equal(result.soa.packetType.serial, 2);
...
test('ixfr#client end-to-end incremental response', async () => {
    ...
    if (result.type === 'incremental') {
        assert.equal(result.currentSoa.packetType.serial, 2);
        ...
        assert.equal(diff.deletions[0].packetType.address, '192.0.2.1');
        assert.equal(diff.additions.length, 2);
        assert.equal(diff.additions[0].packetType.address, '192.0.2.99');
        assert.equal(diff.additions[1].packetType.address, '192.0.2.100');
    }
    await server.close();
});
test('ixfr#client end-to-end multi-step incremental chain', async () => {
    ...
    const intermediate = new PacketResource('example.com', new SOA('ns1.example.com', 'admin.example.com', 3, 7200, 3600, 1209600, 3600), PacketClass.IN, 3600);
    ...
    if (result.type === 'incremental') {
        ...
        assert.equal(result.diffs[1].additions[0].packetType.address, '192.0.2.100');
    }
    await server.close();
});
//# sourceMappingURL=ixfr.js.map