...
const ptrRec = (owner, target) => new PacketResource(owner, new PTR(target), PacketClass.IN, 120);
const srvRec = (owner, target, port) => new PacketResource(owner, new SRV(0, 0, port, target), PacketClass.IN, 120);
const txtRec = (owner, kv) => new PacketResource(owner, new TXT(kv), PacketClass.IN, 120);
const aRec = (owner, addr) => new PacketResource(owner, new A(addr), PacketClass.IN, 120);
const aaaaRec = (owner, addr) => new PacketResource(owner, new AAAA(addr), PacketClass.IN, 120);
const buildPtrReply = (query, answers, additionals = []) => {
    const r = new Packet();
    r.header.qr = 1;
    r.header.aa = 1;
...
test('ServiceDiscovery#browse with resolveMissing:false skips follow-up queries', async () => {
    ...
    try {
        ...
        assert.equal(instances[0].host, undefined, 'host stays undefined when resolveMissing is off and no SRV came in');
