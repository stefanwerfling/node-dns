...
/**
 * Tiny RFC 7766-framed DNS echo server for the pool tests. Reads
 * length-prefixed messages from each connection, parses them, builds
 * a one-A-record response with a configurable answer IP, and writes
 * the framed reply. Optionally delays per-message via `delayMs(message)`
 * so tests can exercise out-of-order pipelining.
 */
type EchoServerOptions = {
    answerIp?: (qname: string) => string;
    delayMs?: (qname: string) => number;
    onConnection?: (socket: tcp.Socket) => void;
};
...
const startEchoServer = (options: EchoServerOptions = {}): Promise<EchoServer> => {
    return new Promise((resolve, reject) => {
        ...
        const server = tcp.createServer((socket) => {
            ...
            socket.on('error', () => {
                /* swallow — peer reset is fine for tests */
            });
        });
        ...
        server.listen(0, '127.0.0.1', () => {
            resolve({
                address: (): AddressInfo => server.address() as AddressInfo,
                connectionCount: (): number => conns,
                close: (): Promise<void> => new Promise((res) => server.close(() => res()))
            });
        });
    });
};

const buildQuery = (name: string, id: number = Math.floor(Math.random() * 0xFFFF)): Buffer => {
    const p = new Packet();
    p.header.id = id;
    p.header.rd = 1;
...
test('TcpConnectionPool#multiplexes pipelined out-of-order responses by ID', async() => {
    // Server replies to "slow" with 30ms delay, "fast" immediately. With
    // pipelining, "fast" should resolve first even though "slow" was sent
    // first.
    const server = await startEchoServer({
        delayMs: (qname): number => qname.startsWith('slow.') ? 30 : 0
    });
    ...
    try {
        // Identical caller-side IDs to prove the pool's internal ID
        // rewrite makes ID collisions safe.
...
test('TcpConnectionPool#opens separate connections for distinct targets', async() => {
    ...
    try {
        const targetA = {protocol: 'tcp' as const, host: '127.0.0.1', port: a.address().port};
        const targetB = {protocol: 'tcp' as const, host: '127.0.0.1', port: b.address().port};
        ...
        assert.strictEqual((replyA.answers[0].packetType as A).address, '10.0.0.1');
        assert.strictEqual((replyB.answers[0].packetType as A).address, '10.0.0.2');
...
test('TcpConnectionPool#idle timeout closes connection after inactivity', async() => {
    ...
    try {
        const target = {protocol: 'tcp' as const, host: '127.0.0.1', port: server.address().port};
        await pool.send(target, buildQuery('first.test.', 1));
        assert.strictEqual(pool.size(), 1);

        // Wait past idle timeout — the pool should drop the connection.
...
test('TcpConnectionPool#reconnects after peer-closed socket', async() => {
    ...
    try {
        ...
        // Bounce the server. Pool's existing connection breaks.
        await server.close();

        // Give the pool a tick to observe the close event.
...
test('TcpConnectionPool#query timeout rejects without breaking other queries', async() => {
    const server = await startEchoServer({
        // First query gets buried; second responds normally. Use a marker
        // qname so the server knows which to drop.
        delayMs: (qname): number => qname.startsWith('blackhole.') ? 1000 : 0
    });
...
test('TcpConnectionPool#close rejects pending queries', async() => {
    ...
    try {
        ...
        // Let the query reach the wire before closing.
        await new Promise((r) => setTimeout(r, 20));
        ...
        await assert.rejects(
            pool.send(target, buildQuery('after.test.', 2)),
            /pool is closed/
        );
    } finally {
        await server.close();
...
test('TcpConnectionPool#asResolverTransport routes resolver-style calls', async() => {
    ...
    try {
        ...
        assert.strictEqual((reply.answers[0].packetType as A).address, '203.0.113.1');
    } finally {
        pool.close();
        await server.close();
    }
...
test('TcpConnectionPool#rejects undersized queries before sending', async() => {
    ...
    try {
        const target = {protocol: 'tcp' as const, host: '127.0.0.1', port: 65535};
        await assert.rejects(
            pool.send(target, Buffer.alloc(8)),
            /too short/
        );
    } finally {
        pool.close();
...
test('TcpConnectionPool#TCPClient honours pool option for end-to-end queries', async() => {
    // Wires the pool through the public TCPClient.request() API, the
    // primary consumer surface. Validates that two calls reuse the
    ...
    try {
        ...
        assert.strictEqual((r1.answers[0].packetType as A).address, '198.51.100.7');
        assert.strictEqual((r2.answers[0].packetType as A).address, '198.51.100.7');
