...
import {StubResolver, StubResolverBackend} from '../Resolver/StubResolver.js';
import {test} from './test.js';

const buildResponse = (qname: string, qtype: PacketTypes | number, rcode: number, answers: PacketResource[] = []): Packet => {
    const p = new Packet();
    p.header.qr = 1;
...
const recordingBackend = (router: (name: string) => Packet): {
    backend: StubResolverBackend;
    calls: string[];
} => {
    const calls: string[] = [];
    const backend: StubResolverBackend = async(name, _type, _cls): Promise<Packet> => {
        calls.push(name);
        return router(name);
    };
...
test('StubResolver#expand returns just the absolute name when name ends in dot', () => {
    const stub = new StubResolver({
        resolver: async(): Promise<Packet> => new Packet(),
...
test('StubResolver#expand short name (dots < ndots) tries search list first then absolute', () => {
    ...
    // 0 dots, ndots=2 — search list first.
    ...
    // 1 dot, ndots=2 — still short → search list first.
...
test('StubResolver#expand normalizes trailing dots in search list and dedupes', () => {
    const stub = new StubResolver({
        resolver: async(): Promise<Packet> => new Packet(),
        search: ['example.com.', 'example.com', 'corp.local.'],   // dup + trailing dots
        ndots: 1
    });
...
test('StubResolver#resolve walks search list on NXDOMAIN and stops on NOERROR', async() => {
    ...
    assert.deepStrictEqual(calls, [
        'host.example.com',   // first search entry → NXDOMAIN, advance
        'host.corp.local'     // second search entry → NOERROR, stop
    ]);
});

test('StubResolver#resolve halts on NODATA (NOERROR with empty answers) — does not advance', async() => {
    const {backend, calls} = recordingBackend((name) => {
        // First candidate replies NOERROR-empty (NODATA). Stub must
        // return it without trying the next search entry.
...
test('StubResolver#resolve long name tries absolute first then search list on NXDOMAIN', async() => {
    ...
    assert.strictEqual(response.header.rcode, RCODE.NOERROR);
    assert.deepStrictEqual(calls, [
        'a.b',                 // dots(1) >= ndots(1) → absolute first
        'a.b.example.com'      // NXDOMAIN on absolute → fall through to search
    ]);
});
