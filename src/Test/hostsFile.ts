...
test('HostsFile.parse skips comments, empty lines, and malformed entries', () => {
    ...
    // Expected valid entries: 127.0.0.1, ::1, 192.168.1.5, 2001:db8::1, 1.2.3.4 spaced
    // Skipped: 'malformed' (single token), '1.2.3.4' (no name), 'not-an-ip foo.bar' (bad address)
    ...
    assert.deepStrictEqual(entries[2].names, ['printer', 'printer.local', 'printer.local'],
        'names lowercased — Printer.LOCAL collapses with printer.local at lookup');
});
...
test('HostsFile#lookup returns A records for IPv4 entries', () => {
    ...
    assert.strictEqual((result.records[0].packetType as A).address, '192.168.1.5');
...
test('HostsFile#lookup returns AAAA records for IPv6 entries', () => {
    ...
    assert.strictEqual((result.records[0].packetType as AAAA).address, '2001:db8::1');
});
...
test('HostsFile#lookup returns both A entries when a name has multiple v4 IPs', () => {
    ...
    assert.strictEqual((result.records[0].packetType as A).address, '10.0.0.1');
    assert.strictEqual((result.records[1].packetType as A).address, '10.0.0.2');
});
...
test('HostsFile#asResolverBackend serves match without consulting fallback', async() => {
    ...
    const fallback = async(): Promise<Packet> => {
        fallbackCalls++;
        const p = new Packet();
        p.header.qr = 1;
        return p;
...
test('HostsFile#asResolverBackend falls through on miss', async() => {
    ...
    const fallback = async(name: string): Promise<Packet> => {
        fallbackCalls++;
        receivedName = name;
        const p = new Packet();
        p.header.qr = 1;
...
test('HostsFile#merge stacks tables, earlier entries win on name collision', () => {
    ...
    // 'web' resolves to the earlier (system) entry first; the lookup
    // returns BOTH because /etc/hosts allows multiple records, but
    // ordering puts system before override.
    ...
    if (resWeb.kind === 'match') {
        ...
        assert.strictEqual((resWeb.records[0].packetType as A).address, '1.1.1.1');
        assert.strictEqual((resWeb.records[1].packetType as A).address, '2.2.2.2');
    }

    // 'lab' only present in local override
...
test('HostsFile.parse handles trailing-comment lines', () => {
    ...
    if (result.kind === 'match') {
        assert.strictEqual((result.records[0].packetType as A).address, '1.2.3.4');
    }
});