...
test('Dso.KeepaliveTlv encode/decode round-trip', () => {
    ...
    assert.equal((decoded.tlvs[0] as KeepaliveTlv).inactivityMs, 60_000);
    assert.equal((decoded.tlvs[0] as KeepaliveTlv).keepaliveMs, 5_000);
});
...
test('Dso.RetryDelayTlv encode/decode round-trip', () => {
    ...
    assert.equal((decoded.tlvs[0] as RetryDelayTlv).retryDelayMs, 120_000);
});
...
test('Dso.EncryptionPaddingTlv preserves arbitrary bytes', () => {
    ...
    const pad = decoded.tlvs[1] as EncryptionPaddingTlv;
...
test('Dso.SubscribeTlv encodes name + qtype + qclass uncompressed', () => {
    ...
    const got = decoded.tlvs[0] as SubscribeTlv;
...
test('Dso.UnsubscribeTlv carries the original SUBSCRIBE message id', () => {
    ...
    const got = decoded.tlvs[0] as UnsubscribeTlv;
...
test('Dso.PushTlv round-trip carries answer records', () => {
    ...
    const got = decoded.tlvs[0] as PushTlv;
    assert.ok(got instanceof PushTlv);
    assert.equal(got.records.length, 2);
    assert.equal(got.records[0].name, 'host.example.com');
    assert.equal((got.records[0].packetType as A).address, '192.0.2.1');
    assert.equal((got.records[1].packetType as AAAA).address, '2001:db8::1');
});
...
test('Dso.ReconfirmTlv round-trip preserves NAME + TYPE + CLASS + RDATA', () => {
    ...
    const got = decoded.tlvs[0] as ReconfirmTlv;
...
test('Dso message has opcode=6 in the wire header', () => {
    ...
    // Byte 2 holds (qr|opcode|aa|tc|rd) — QR=0 (request), opcode=6.
    // Layout: 0(qr) 0110(opcode) 0(aa) 0(tc) 0(rd) = 0b00110000 = 0x30.
...
test('Dso TLV wire format is type|length|data with 16-bit big-endian fields', () => {
    ...
    // Header is 12 bytes; first TLV starts at offset 12.
...
test('Dso UnknownDsoTlv round-trips an unrecognised type code', () => {
    ...
    const got = decoded.tlvs[0] as UnknownDsoTlv;
...
test('Dso message round-trips multiple TLVs in declaration order', () => {
    const message = DsoMessage.request(
        100,
        new KeepaliveTlv(60_000, 5_000),
        new EncryptionPaddingTlv(Buffer.from([0xFF, 0xFF])),
        new UnknownDsoTlv(0xCAFE, Buffer.from([0xDE, 0xAD]))
    );
    const decoded = DsoMessage.parse(message.toBuffer());
