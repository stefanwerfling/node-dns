...
/**
 * Sample wire-format DNS response (www.z.cn IN A) used by parser roundtrip tests.
 */
...
/**
 * Minimal HTTP GET helper used by DoH CORS tests.
 */
export const get = (url: string, options?: http.RequestOptions): Promise<{body: Buffer; headers: http.IncomingHttpHeaders;}> => {
...
/**
 * Send a DNS query over TLS and parse the length-prefixed response.
 * Skips chain verification — the bundled example cert is self-signed and expired.
 */
export const dotQuery = (port: number, name: string): Promise<Packet> => {
    return new Promise((resolve, reject) => {
