...
test('zone#recordsOfType filters by type', () => {
    ...
    assert.equal((aRecords[1].packetType as A).address, '192.0.2.2');
});
...
test('zone#toAxfrPackets emits SOA-bracketed answer (RFC 5936 §2.2)', () => {
    ...
    // Answer section: SOA, then every other record (4), then SOA again.
...
test('axfr#end-to-end via DnsServer + AxfrClient', async() => {
    ...
    // The opening SOA is included in `records`, the closing SOA is stripped.
    assert.equal(result.records.length, zone.records.length);
    assert.equal((result.soa.packetType as SOA).serial, 2024010101);

    // First record returned is the SOA, then NS/MX/A/A in some order.
...
test('axfr#client errors on premature close', async() => {
    const server = new DnsServer({
        tcp: true,
        handle: (request, send): void => {
            // Build a malformed AXFR response: SOA-only message, no closing SOA.
            // The server signals "done" by closing the connection, so the client
            // should error out (it never saw the second SOA).
            const zone = Zone.fromZoneFile(SAMPLE_ZONE);
...
test('server/tcp#multi-packet send writes all frames', async() => {
    // Direct test of the new send(Packet[]) API: server hands two packets,
    // client receives both length-prefixed frames before EOF.
    const server = new DnsServer({
        tcp: true,
        handle: (request, send): void => {
            // Build two independent responses. createResponseFromRequest
            // shares the header object with the request, so we use plain
            // Packet() and copy what we need to keep frames distinguishable.
            const a = new Packet();
            a.header.id = 1;
            a.header.qr = 1;
    ...
    // Two length-prefixed frames in `all`.
