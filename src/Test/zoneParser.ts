...
test('zone#HINFO parses cpu and os', () => {
    const {records} = ZoneParser.parse(
        'host 60 IN HINFO "x86_64" "Linux"',
        {origin: 'example.com.'}
    );
    const hinfo = records[0].packetType as HINFO;
...
test('zone#URI parses priority weight target', () => {
    const {records} = ZoneParser.parse(
        '_ftp._tcp 60 IN URI 10 1 "ftp://ftp.example.com/public"',
        {origin: 'example.com.'}
    );
    const uri = records[0].packetType as URI;
...
test('zone#ZONEMD parses serial scheme algorithm digest', () => {
    ...
    const {records} = ZoneParser.parse(
        `@ 86400 IN ZONEMD 2024010101 1 1 ${digest}`,
        {origin: 'example.com.'}
    );
    const zonemd = records[0].packetType as ZONEMD;
...
test('zone#OPENPGPKEY parses base64 to hex', () => {
    // 6 bytes "Hello!" → base64 "SGVsbG8h", hex "48656c6c6f21"
    const {records} = ZoneParser.parse(
        'c93f1e4._openpgpkey 3600 IN OPENPGPKEY SGVsbG8h',
        {origin: 'example.com.'}
    );
    const key = records[0].packetType as OPENPGPKEY;
    assert.equal(key.publicKey, '48656c6c6f21');
});

test('zone#SMIMEA parses usage selector matching cert', () => {
    const {records} = ZoneParser.parse(
        'c93f1e4._smimecert 3600 IN SMIMEA 3 1 1 aabbccdd00112233',
        {origin: 'example.com.'}
    );
    const smimea = records[0].packetType as SMIMEA;
...
test('zone#CERT parses type keyTag algorithm base64-cert', () => {
    // 6 bytes "Hello!" → base64 "SGVsbG8h"
    const {records} = ZoneParser.parse(
        'cert 3600 IN CERT 3 12345 8 SGVsbG8h',
        {origin: 'example.com.'}
    );
    const cert = records[0].packetType as CERT;
...
test('zone#LOC parses raw seven-field integer form', () => {
    const {records} = ZoneParser.parse(
        'loc 3600 IN LOC 0 18 22 19 2295968000 1404416000 10000500',
        {origin: 'example.com.'}
    );
    const loc = records[0].packetType as LOC;
