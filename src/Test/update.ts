...
test('update#builder produces opcode=UPDATE with right sections', () => {
    ...
    assert.equal(packet.answers.length, 1);          // 1 prerequisite
    assert.equal(packet.authorities.length, 1);      // 1 update
});
...
test('update#wire-format roundtrip preserves rdlength=0 markers', () => {
    ...
    // Empty-RDATA markers must roundtrip with rdlength=0.
...
test('update#applyToZone adds a record', () => {
    ...
    assert.equal((added.packetType as A).address, '192.0.2.10');
});
...
test('update#applyToZone refreshes TTL on existing matching RR', () => {
    ...
    // Re-add the same RR with a different TTL — RFC 2136 §3.4.2.5 says the
    // TTL is replaced and no duplicate is added.
    const builder = new UpdateBuilder('example.com')
...
test('update#applyToZone deletes by RRset', () => {
    const zone = Zone.fromZoneFile(ZONE);
    // Add an AAAA so www has both A and AAAA.
...
test('update#applyToZone deletes a specific RR', () => {
    ...
    const wwwAddrs = zone.records
        ...
        .map((r) => (r.packetType as A).address);
...
test('update#applyToZone returns NXDOMAIN when nameInUse fails', () => {
    ...
    // Atomicity: failed prerequisite must not have applied the update.
...
test('update#end-to-end UDP via DnsServer + UpdateClient', async() => {
    ...
    const fail = await send(new UpdateBuilder('example.com')
        .requireNameNotInUse('www.example.com')                       // currently exists
...
/**
 * Send raw bytes over UDP and resolve with the raw response bytes. UpdateClient
 * discards the raw response, but the test needs them to verify the response MAC.
 */
const sendRawUdp = (port: number, payload: Buffer): Promise<Buffer> => {
...
test('update#TSIG-signed UPDATE end-to-end (server verifies + signs reply)', async() => {
    ...
    const server = new DnsServer({
        udp: true,
        // 4th arg is the raw post-preRequest buffer — required by TSIG, since
        // re-encoding the parsed packet may produce different name-compression
        // bytes than the MAC was computed over.
        handle: (request, send, _client, raw): void => {
    ...
    const addresses = await server.listen();
    const port = (addresses.udp as AddressInfo).port;
    ...
    // Verify response MAC — chains from request MAC, uses the raw wire bytes.
...
test('update#TSIG-signed UPDATE rejects wrong-secret with BADSIG', async() => {
    ...
    // Server should have appended a TSIG with BADSIG.
    ...
    assert.equal((lastAdd.packetType as TSIG).error, TsigError.BADSIG);
