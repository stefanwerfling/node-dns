...
const a = (name: string, addr: string, ttl: number = 3600): PacketResource =>
...
test('DnsCache#different types are separate entries', () => {
    const cache = new DnsCache();
    cache.set('host.example.com', PacketTypes.A, PacketClass.IN,
        [a('host.example.com', '192.0.2.1')], 60);
    cache.set('host.example.com', PacketTypes.AAAA, PacketClass.IN,
        [new PacketResource('host.example.com', new AAAA('2001:db8::1'), PacketClass.IN, 60)], 60);
...
test('DnsCache#expired entries return null and are evicted', () => {
    ...
    now += 60_000 + 1; // just past expiry
    assert.equal(cache.get('www.example.com', PacketTypes.A, PacketClass.IN), null);
    assert.equal(cache.size(), 0); // expired entry was deleted on lookup
});

test('DnsCache#set with maxTtl clamps long TTLs', () => {
    const cache = new DnsCache({maxTtlSeconds: 60, now: (): number => 0});
...
test('DnsCache#set with minTtl floors short TTLs', () => {
    const cache = new DnsCache({minTtlSeconds: 5, now: (): number => 0});
...
test('DnsCache#replacing an entry resets its expiry', () => {
    ...
    // Original would expire at 1_060_000; advance to 1_080_000 — past
    // original expiry but still inside the refreshed entry's window
    // (refreshed at 1_030_000 + 60_000 = 1_090_000).
    now = 1_080_000;
    const got = cache.get('www.example.com', PacketTypes.A, PacketClass.IN);
    assert.ok(got);
    assert.equal((got!.records[0].packetType as A).address, '192.0.2.2');
});
...
test('DnsCache#serve-stale returns expired entries with stale=true within the window', () => {
    ...
    cache.set('www.example.com', PacketTypes.A, PacketClass.IN, [a('www.example.com', '192.0.2.1')], 60);
    now += 70_000; // 10s past expiry, still inside the 300s stale window
...
test('DnsCache#serve-stale evicts past the max-stale window', () => {
    ...
    cache.set('www.example.com', PacketTypes.A, PacketClass.IN, [a('www.example.com', '192.0.2.1')], 60);
    // Past expiresAt (60s) + maxStale (300s) = 360s past insert.
    now += 400_000;
    assert.equal(cache.get('www.example.com', PacketTypes.A, PacketClass.IN), null);
});
...
test('DnsCache#serve-stale leaves fresh entries untagged', () => {
    ...
    cache.set('www.example.com', PacketTypes.A, PacketClass.IN, [a('www.example.com', '192.0.2.1')], 60);
    now += 30_000; // still inside the original 60s TTL
