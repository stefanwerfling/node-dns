...
const buildQuery = (id: number, name: string, cookieOption?: EdnsCookie): Buffer => {
    const p = new Packet();
    p.header.id = id;
    p.header.rd = 1;
...
const tcpQuery = (port: number, payload: Buffer): Promise<Packet> => {
...
const tlsQuery = (port: number, payload: Buffer): Promise<Packet> => {
    return new Promise((resolve, reject) => {
...
const startTcpServer = async(cookies: ServerCookieOptions): Promise<TcpHandle> => {
...
test('TCPServer cookies: strict mode REFUSES queries without cookie option', async() => {
    ...
    try {
        ...
        assert.equal(resp.header.rcode, 5 /* REFUSED */);
        assert.equal(handle.received.length, 0);
...
test('TLSServer cookies: end-to-end DoT BADCOOKIE + retry path', async() => {
    ...
    try {
        const clientCookie = EdnsCookie.generateClientCookie();
        // First: client-only cookie → BADCOOKIE
        ...
        // Second: retry with learned cookie → handler runs
...
test('TCPClient cookies: shared jar with UDPClient reuses same cookie pair (cross-transport)', async() => {
    // UDP and TCP servers share the same secret + listen on different
    // ports — but to test cross-transport identity, we only need to
    // verify the jar's stable client cookie is used by both clients.
    const handle = await startTcpServer({secret: SECRET});

    try {
        const jar = new ClientCookieJar();
        // Prime jar by way of a TCP query.
        ...
        // Second TCP query — same jar, same cookie pair, no new BADCOOKIE.
...
test('TCPClient cookies: works over DoT (TLS transport)', async() => {
    ...
    try {
        // Self-signed cert: we need to provide a TLS option override.
        // TCPClient doesn't expose that today, so set NODE_TLS_REJECT_UNAUTHORIZED.
