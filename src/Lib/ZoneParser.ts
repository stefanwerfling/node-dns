    ...
    protected static _parseRdata(typeStr: string, rdata: ZoneToken[], origin: string, lineNumber: number): PacketType {
        ...
        switch (typeStr) {
            ...
            case 'OPENPGPKEY': {
                ...
                // RFC 7929 presentation form is base64; we keep the
                // RDATA as hex internally for parity with DS/TLSA.
            ...
            case 'CERT': {
                ...
                const algorithm = parseInt(v(2), 10);
                // Cert presentation form is base64; store as hex internally.
            ...
            case 'LOC': {
                // RFC 1876 presentation form is complex (deg/min/sec). We
                // keep this parser to the raw-integer form for now: seven
                // unsigned-decimal tokens matching the class's constructor
                // (version size horizPre vertPre lat lon alt). Callers
                // that need the deg/min/sec parsing can pre-process.
